...
describe("wallet store", () => {
  ...
  describe("BIP321 on-chain payment amounts", () => {
    ...
    let wallet;
    let createMeltQuoteOnchain;

    beforeEach(() => {
      h.mintsStore.activeUnitCurrencyMultiplyer = 1;
      h.mintsStore.mints[0].info.nuts = {
        5: {
          methods: ["sat", "msat", "usd"].map((unit) => ({
            method: "onchain",
            unit,
          })),
        },
      ...
      createMeltQuoteOnchain = vi.fn(async (request, amount) => ({
        quote: "onchain-melt-q",
        request,
        amount,
        fee_reserve: 1,
      ...
      vi.spyOn(wallet, "activeWallet").mockImplementation(async () => ({
        unit: h.mintsStore.activeUnit,
    ...
    it.each([
      [`bitcoin:${address}?amount=0.00000029`, address],
      [`BITCOIN:${address}?AMOUNT=0.00000029`, address],
      [`bitcoin:?tb=${testnetAddress}&amount=0.00000029`, testnetAddress],
    ])(
      "quotes the URI amount and waits for Pay: %s",
      async (uri, expectedAddress) => {
        await wallet.decodeRequest(uri);
        expect(wallet.payInvoiceData.input.amount).toBe(29);
        expect(wallet.payInvoiceData.invoice.onchainAmountSat).toBe(29);
        expect(createMeltQuoteOnchain).toHaveBeenCalledExactlyOnceWith(
          expectedAddress,
          29
        );
        expect(wallet.payInvoiceData.meltQuote.response.amount).toBe(29);
        expect(wallet.meltInvoiceData).not.toHaveBeenCalled();
      }
    );

    it.each([address, `bitcoin:${address}`, `bitcoin:${address}?label=Alice`])(
      "still asks for an amount when none is present: %s",
      async (uri) => {
        wallet.payInvoiceData.input.amount = 1000;
        await wallet.decodeRequest(uri);
        expect(wallet.payInvoiceData.input.amount).toBeUndefined();
        expect(wallet.payInvoiceData.invoice.onchainAmountSat).toBeUndefined();
        expect(createMeltQuoteOnchain).not.toHaveBeenCalled();
        wallet.payInvoiceData.input.amount = 17;
        await wallet.meltQuoteInvoiceData();
        expect(createMeltQuoteOnchain).toHaveBeenCalledExactlyOnceWith(
          address,
          17
        );
    ...
    it("preserves the Bitcoin value when changing the selected unit", async () => {
      await wallet.decodeRequest(`bitcoin:${address}?amount=0.00000029`);
      h.mintsStore.activeUnit = "msat";
      await wallet.meltQuoteInvoiceData();
      expect(createMeltQuoteOnchain).toHaveBeenLastCalledWith(address, 29000);
      expect(wallet.payInvoiceData.input.amount).toBe(29000);
      h.mintsStore.activeUnit = "sat";
      await wallet.meltQuoteInvoiceData();
      expect(createMeltQuoteOnchain).toHaveBeenLastCalledWith(address, 29);
      expect(wallet.payInvoiceData.input.amount).toBe(29);
    });

    it("does not reinterpret BTC as fiat", async () => {
      h.mintsStore.activeUnit = "usd";
      await wallet.decodeRequest(`bitcoin:${address}?amount=0.00000029`);
      expect(createMeltQuoteOnchain).not.toHaveBeenCalled();
      expect(wallet.payInvoiceData.meltQuote.error).toContain("sat or msat");
    });

    it("rejects an amount that would overflow the selected msat unit", async () => {
      h.mintsStore.activeUnit = "msat";
      await wallet.decodeRequest(`bitcoin:${address}?amount=90071992.54740991`);
      expect(createMeltQuoteOnchain).not.toHaveBeenCalled();
      expect(wallet.payInvoiceData.meltQuote.error).toContain("out of range");
    });
    ...
    it.each([
      ...
      "rejects %s without falling back to a bare address or a stale quote",
      async (query) => {
        wallet.payInvoiceData.input.amount = 1000;
        wallet.payInvoiceData.meltQuote.response = {
          quote: "old-q",
        ...
        expect(wallet.payInvoiceData.input.amount).toBeUndefined();
        expect(wallet.payInvoiceData.meltQuote.response.quote).toBe("");
    ...
    it("keeps the amount when quote creation fails and does not retry without it", async () => {
      createMeltQuoteOnchain.mockRejectedValue(new Error("Mint unavailable"));
      await wallet.decodeRequest(`bitcoin:${address}?amount=0.00000029`);
      expect(wallet.payInvoiceData.input.amount).toBe(29);
      expect(wallet.payInvoiceData.invoice.onchainAmountSat).toBe(29);
      expect(createMeltQuoteOnchain).toHaveBeenCalledOnce();
      expect(wallet.payInvoiceData.meltQuote.error).toBe("Mint unavailable");
      expect(h.notifyApiError).toHaveBeenCalledOnce();
    });

    it("does not request a quote without an on-chain capable mint", async () => {
      h.mintsStore.mints[0].info.nuts[5].methods = [];
      await wallet.decodeRequest(`bitcoin:${address}?amount=0.00000029`);
      expect(createMeltQuoteOnchain).not.toHaveBeenCalled();
      expect(wallet.payInvoiceData.meltQuote.error).toBe(
